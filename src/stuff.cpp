...
//! The start condition is only valid if SCL goes LOW t_HSTART after SDA goes LOW, smae with stop
//! There has to be a clock pulse done for ACK check after sending byte
// Each bit during each SCL pulse in the SDA must be kept stable when SCL is HIGH 
//      Except during start or stop where data can be switched when SCL is LOW 

/*
    8 Pages
    128 x 64, so each page 8 pixels high
    Each Page has 128 segments, (horizontal)
                Column 0   Column 1   Column 2  ... Column 127
              +----------+----------+----------+
        Row 0 | Bit D0=1 | Bit D0=0 | Bit D0=0 |
        Row 1 | Bit D1=0 | Bit D1=0 | Bit D1=0 |
        Row 2 | Bit D2=0 | Bit D2=0 | Bit D2=0 |   <-- This entire block 
        Row 3 | Bit D3=0 | Bit D3=0 | Bit D3=0 |       is PAGE 0 
        Row 4 | Bit D4=0 | Bit D4=0 | Bit D4=0 |       (Y: 0 to 7)
        Row 5 | Bit D5=0 | Bit D5=0 | Bit D5=0 |
        Row 6 | Bit D6=0 | Bit D6=0 | Bit D6=0 |
        Row 7 | Bit D7=0 | Bit D7=0 | Bit D7=0 |
              +----------+----------+----------+
*/
...
void S1306_init(){
    ...
    u8 controlByte = CMD_MODE;    // 0x80; (command mode)
...
void i2cWrite(u8 data){
    ...
    if (readAck()); // ion really care if ACK or not 
}

void i2cStart(){
    sclReleaseLine();  // make sure scl is high
    sdaLow();
    delayMicroseconds(1);  // t_HSTART must be at least 0.6 microseconds
    sclLow();
}
...
void i2cStop(){
    ...
    delayMicroseconds(1);   // tsStop is 0.6 micros
    sdaReleaseLine();
}
